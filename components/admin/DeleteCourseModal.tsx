...
export default function DeleteCourseModal({ 
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-80 flex items-center justify-center z-50 p-4">
      <div className="bg-[#1A1A1A] text-white rounded-xl border-2 border-red-500/50 p-8 w-full max-w-lg text-center transform transition-all animate-in fade-in-0 zoom-in-95">
        ...
        <div className="flex justify-end items-center gap-4">
          <button onClick={onClose} className="py-2 px-4 text-gray-300 font-medium rounded-lg hover:bg-gray-700">
            Cancelar
          </button>
          <button
            onClick={onConfirm}
            disabled={!isConfirmationMatch || isDeleting}
            className="flex items-center gap-2 bg-red-600 text-white font-bold py-2 px-4 rounded-lg hover:bg-red-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
          >
            <Trash2 size={18} />
