// Ruta: app/admin/cursos/editar/[courseId]/leccion/[lessonId]/page.tsx
...
export default function LessonEditPage({ params }: { params: { courseId: string, lessonId: string } }) {
  ...
  // --- NUEVA FUNCIÓN PARA BORRAR EL QUIZ ---
  const handleDeleteQuiz = async () => {
    ...
    if (window.confirm("¿Estás seguro de que querés borrar este quiz y todas sus preguntas? Esta acción es irreversible.")) {
        ...
        if (error) {
            ...
            // Si se borra con éxito, actualizamos el estado para que desaparezca de la UI
            setQuiz(null);
        }
    }
  };

  if (loading) return <div className="min-h-screen bg-[#0D0D0D] flex items-center justify-center text-white"><p>Cargando datos...</p></div>;
    ...
    <>
      ...
      <div className="min-h-screen bg-[#0D0D0D] text-gray-200 p-8">
        <div className="max-w-4xl mx-auto space-y-8">
          <header>
            <Link href={`/admin/cursos/editar/${courseId}`} className="flex items-center gap-2 text-gray-400 hover:text-white">
