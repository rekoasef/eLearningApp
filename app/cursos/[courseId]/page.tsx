...
export default function CourseDetailPage({ params }: { params: { courseId: string } }) {
  ...
  const fetchCourseData = useCallback(async () => {
    ...
    // Si el curso es futuro, solo necesitamos la data básica.
    if (availability === 'UPCOMING') {
        setCourse({ ...courseData, lessons: [] }); // Asignamos lessons vacío
        setLoading(false);
        return;
    }
    ...
    // Asignamos la data de curso y lecciones
  ...
  const ModuleItem = ({ lesson }: { lesson: Lesson }) => {
    ...
    const isUnlocked = lesson.order === 1 || (prevLessonId ? completedLessonIds.has(prevLessonId) : false);
    ...
    return isUnlocked ? <Link href={`/cursos/${courseId}/leccion/${lesson.id}`}>{content}</Link> : <div key={lesson.id}>{content}</div>;
  };
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <main>
            {course ? (
                ...
                {/* --- SECCIÓN PRINCIPAL AHORA SE MUESTRA SI NO ES 'UPCOMING' --- */}
                {courseAvailability !== 'UPCOMING' && (
                        ...
                        {isCourseFinished && (
                            <div className="bg-red-900/50 border border-red-500/50 text-red-300 p-4 rounded-lg flex items-center gap-4">
                                ...
                                <div>
                                    ...
                                    <p className="text-sm">Ya no puedes realizar el examen, pero puedes revisar el contenido.</p>
                                </div>
                            </div>
                        )}
                                ...
                                {courseStatus === 'completed' ? (
                                    <div>
                                        ...
                                        <div className="flex flex-wrap gap-4">
                                            ...
                                            <Link href={`/cursos/${courseId}/examen/${finalExam.id}/revision`} className="inline-flex items-center gap-2 bg-gray-600 text-white font-bold py-2 px-4 rounded-lg hover:bg-gray-500"><Eye size={18}/> Revisar Examen</Link>
                                        </div>
                                    </div>
                                ) : courseStatus === 'failed' ? (
                                    <div>
                                    ...
                                    <div>
                                        ...
                                        <Link href={!isCourseFinished ? `/cursos/${courseId}/examen/${finalExam.id}` : '#'} className={`inline-block font-bold py-3 px-6 rounded-lg ${ isCourseFinished ? 'bg-gray-600 text-gray-400 cursor-not-allowed' : 'bg-amber-500 text-white hover:bg-amber-600' }`} aria-disabled={isCourseFinished} onClick={(e) => isCourseFinished && e.preventDefault()}>
