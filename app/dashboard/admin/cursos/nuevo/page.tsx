...
export default function NewCoursePage() {
  ...
  const handleSaveCourse = async (e: React.FormEvent) => {
    ...
    const { error: insertError } = await supabase.from('courses').insert({
      title, description, sector_id: finalSectorId, is_published: isPublished,
      creator_id: user.id, start_date: startDate || null, end_date: endDate || null,
    });
    if (insertError) {
  ...
  const handleGenerateContent = async () => {
    if (!title) { alert("Por favor, primero escribe un título para el curso."); return; }
    setIsGenerating(true);
    setGenerationStatus(null);
    try {
      const { data, error } = await supabase.functions.invoke('generate-course-content', { body: { mode: 'details', title } });
  ...
  return (
    <div className="text-gray-200 p-8">
      <div className="max-w-4xl mx-auto">
        <header className="mb-4"><h1 className="text-3xl font-bold text-white">Crear Nuevo Curso</h1></header>
        <form onSubmit={handleSaveCourse} className="bg-[#151515] rounded-xl border border-gray-800 p-8 space-y-6">
          <div>
            ...
            <input id="title" value={title} onChange={(e) => setTitle(e.target.value)} className="w-full px-4 py-2 bg-[#0D0D0D] border border-gray-600 rounded-md" required />
          </div>
          <div>
            <div className="flex justify-between items-center mb-2">
                <label htmlFor="description" className="block text-sm font-medium text-gray-300">Descripción y Temario</label>
                <button type="button" onClick={handleGenerateContent} disabled={isGenerating} className="flex items-center gap-2 text-xs bg-purple-600 text-white font-bold py-1 px-3 rounded-lg hover:bg-purple-700 disabled:opacity-50">
                  {isGenerating ? <Loader2 size={16} className="animate-spin" /> : <Sparkles size={16} />}
                  {isGenerating ? 'Generando...' : 'Generar con IA'}
                </button>
            </div>
             {generationStatus === 'success' && (<div className="my-2 p-2 bg-green-900/50 text-green-300 text-sm rounded-md flex items-center gap-2"><CheckCircle size={16} /> Contenido generado con éxito.</div>)}
            {generationStatus === 'error' && (<div className="my-2 p-2 bg-red-900/50 text-red-300 text-sm rounded-md flex items-center gap-2"><XCircle size={16} /> Hubo un error al generar el contenido.</div>)}
            <textarea id="description" value={description} onChange={(e) => setDescription(e.target.value)} rows={10} placeholder="Escribe una descripción o usa la IA para generarla junto con un temario." className="w-full px-4 py-2 bg-[#0D0D0D] border border-gray-600 rounded-md" />
          </div>
          {userProfile?.role_id === 1 ? (
            <div>
              <label htmlFor="sector" className="block text-sm font-medium text-gray-300 mb-2">Sector</label>
              <select id="sector" value={sectorId} onChange={(e) => setSectorId(e.target.value)} className="w-full px-4 py-2 bg-[#0D0D0D] border border-gray-600 rounded-md" required>
                <option value="" disabled>Selecciona un sector...</option>
                {allSectors.map(sector => (<option key={sector.id} value={sector.id}>{sector.name}</option>))}
              </select>
            </div>
          ) : (
            <div>
