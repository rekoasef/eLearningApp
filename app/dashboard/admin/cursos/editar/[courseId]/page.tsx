// Ruta: app/dashboard/admin/cursos/editar/[courseId]/page.tsx
      ...
      <div className="text-gray-200 p-8">
        <div className="max-w-4xl mx-auto space-y-8">
          ...
          <div className="bg-[#151515] rounded-xl border border-gray-800 p-8">
            ...
            <div className="space-y-3">{lessons.length > 0 ? lessons.map(lesson => (
              <div key={lesson.id} className="flex items-center justify-between bg-gray-800/70 p-4 rounded-lg group">
                <Link href={`/dashboard/admin/cursos/editar/${courseId}/leccion/${lesson.id}`} className="flex-grow">
                  <p className="font-medium text-white group-hover:text-[#FF4500] transition-colors">{lesson.order}. {lesson.title}</p>
                </Link>
                <div className="flex gap-4">
                  <Link href={`/dashboard/admin/cursos/editar/${courseId}/leccion/${lesson.id}`} className="text-gray-400 hover:text-white">
                    <Edit size={18}/>
                  </Link>
                  <button onClick={() => handleDeleteLesson(lesson.id)} className="text-gray-400 hover:text-red-500"><Trash2 size={18}/></button>
                </div>
              </div>
              )) : (!isCreatingLesson && <p className="text-gray-500 text-center py-4">Este curso aún no tiene módulos.</p>)}
            </div>
          </div>
